...
// all required inside each array entry, so there is no cross-field pairing left to validate at
// combat time (only the no-duplicate-stat-per-target rule, enforced in CombatFunction.ApplyBuffsDebuffs).
