...
// schema marks stat/type/target/rounds/untilRemoved/cancelOnEntityDeath/cancelOnApplierDeath all
// required inside each array entry, so there is no cross-field pairing left to validate at combat
// time (only the no-duplicate-stat-per-target rule, enforced in CombatFunction.ApplyRegensDrains).
// Mirrors BuffDebuffSpec field for field.
