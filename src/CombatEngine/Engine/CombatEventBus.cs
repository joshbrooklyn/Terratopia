...
public static class CombatEventBus
{
    ...
    // A triggered effect granted mid-combat by an action's triggeredEffectsApplied rider. Only
    // raised when the entity did not already own it - TriggeredEffectTracker.Add is a no-op on a
    // re-grant, and that no-op is not reported here. Triggered effects never tick or expire on
    // their own, so unlike BuffDebuffApplied/RegenDrainApplied there is no Ticked counterpart -
    // only the Applied/Removed pair below.
    public static event Action<string, string, string, string, string>? TriggeredEffectApplied; // entityId, entityName, triggeredEffectName, sourceId, sourceName
    // A triggered effect stripping its own ownership (e.g. LivingDead consuming itself on death).
    // Only raised on a genuine removal - TriggeredEffectTracker.Remove is a no-op if the entity
    // didn't own the triggered effect, and that no-op is not reported here. No sourceId/sourceName:
    // removal is always the triggered effect acting on itself, so there is no separate causing
    // Tech/Item/MonsterAction to name.
    public static event Action<string, string, string>? TriggeredEffectRemoved; // entityId, entityName, triggeredEffectName
    ...
    internal static void RaiseTriggeredEffectApplied(string entityId, string entityName, string triggeredEffectName, string sourceId, string sourceName) => TriggeredEffectApplied?.Invoke(entityId, entityName, triggeredEffectName, sourceId, sourceName);
    internal static void RaiseTriggeredEffectRemoved(string entityId, string entityName, string triggeredEffectName) => TriggeredEffectRemoved?.Invoke(entityId, entityName, triggeredEffectName);
    ...
    public static void Reset()
    {
        ...
        TriggeredEffectApplied = null;
        TriggeredEffectRemoved = null;
